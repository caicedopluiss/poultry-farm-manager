...
public sealed class GetProductVariantsByProductIdQuery
{
    ...
    public sealed class Handler(IUnitOfWork unitOfWork) : AppRequestHandler<Args, Result>
    {
        ...
        {
            var productVariants = await unitOfWork.ProductVariants.GetByProductIdAsync(args.ProductId, cancellationToken);
